...
class SpriteIsolator:
    ...
    def detect_matrix_layout(grid_img: Image.Image) -> tuple[str, int, int]:
        ...
        row_energies = [
            int(np.sum(alpha[round(r * h / 4) : round((r + 1) * h / 4), :] > 0))
            for r in range(4)
        ]
        ...
        for r in range(4):
            cols = [
                int(
                    np.sum(
                        alpha[
                            round(r * h / 4) : round((r + 1) * h / 4),
                            round(c * w / 4) : round((c + 1) * w / 4),
                        ]
                        > 0
                    )
                )
            ...
            m4.append(cols)
        m4_arr = np.array(m4)
        cut_4 = sum(int(np.sum(alpha[:, round(c * w / 4)] > 0)) for c in range(1, 4))
        ...
        for r in range(4):
            ...
            m5.append(cols)
        m5_arr = np.array(m5)
        cut_5 = sum(int(np.sum(alpha[:, round(c * w / 5)] > 0)) for c in range(1, 5))
    ...
    def isolate_grid_cells(self, grid_img: Image.Image, n_rows: int = 4, n_cols: int = 4) -> list[list[FrameItem]]:
        ...
        for r in range(n_rows):
            ...
            y1 = round(r * img_h / n_rows)
            y2 = round((r + 1) * img_h / n_rows)
            for c in range(n_cols):
                x1 = round(c * img_w / n_cols)
                x2 = round((c + 1) * img_w / n_cols)
                cell_w = x2 - x1
                cell_h = y2 - y1
