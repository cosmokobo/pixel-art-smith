...
PALETTES: dict[str, list[str]] = {
    ...
    "gameboy-pocket": ["#2b2b2b", "#545454", "#878787", "#c2c2c2"],
    "gameboy-color": [
        "#000000",
        "#181010",
        "#282020",
...
class PaletteQuantizer:
    """CIELAB-based perceptual color quantization with Luminance/Contrast Ramp Guards."""

    def __init__(self, palette_name: str = "endesga-32", custom_colors: list[str] | None = None):
        self.palette_name = palette_name.lower()
        self._palette_rgb: np.ndarray | None = None
        self._palette_lab: np.ndarray | None = None
    ...
    def quantize(self, img: Image.Image) -> Image.Image:
        """Quantize an RGBA PIL image to the active palette with outline & contrast preservation.
        ...
        # Vectorized Euclidean distance in CIELAB space (L, A, B)
        # dist shape: (N_pixels, N_palette_colors)
        # Weight L channel slightly higher to prevent dark lines from jumping to light tones
        diff_L = lab_pixels[:, np.newaxis, 0] - self._palette_lab[np.newaxis, :, 0]
        diff_A = lab_pixels[:, np.newaxis, 1] - self._palette_lab[np.newaxis, :, 1]
        diff_B = lab_pixels[:, np.newaxis, 2] - self._palette_lab[np.newaxis, :, 2]

        dists = (1.5 * diff_L) ** 2 + diff_A**2 + diff_B**2

        # Outline protection: If pixel is very dark (L < 40), heavily penalize bright palette colors
        is_dark = lab_pixels[:, 0] < 40
        palette_is_bright = self._palette_lab[:, 0] > 60
        dists[is_dark[:, np.newaxis] & palette_is_bright[np.newaxis, :]] += 20000.0
